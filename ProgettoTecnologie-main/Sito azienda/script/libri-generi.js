...
async function displayGenres(findGenres) {
    ...
    findGenres.forEach(genre => {
        ...
        let totalBooksWidth = 0; // Total width of all books
        books.forEach(book => {
            if (book.genreName === genre) {
                ...
                // Update the total width of all books
                totalBooksWidth += bookDiv.offsetWidth;
            }
        });
        ...
        // Check if the total width of books exceeds the container's width
        const containerWidth = document.getElementById("generi").offsetWidth;
        if (totalBooksWidth > containerWidth) {
            // Apply coriolis effect only if total width exceeds container's width
            booksContainer.classList.add("coriolis");
            // Set animation duration based on the ratio of container's width to total books' width
            const animationDuration = (totalBooksWidth / containerWidth) * 10 + 's'; // Adjust the tempo as needed
            booksContainer.style.animationDuration = animationDuration;
        }
    });
}
